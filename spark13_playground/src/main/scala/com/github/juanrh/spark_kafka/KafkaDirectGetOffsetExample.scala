...
object KafkaDirectExample extends App {   
  ...
  case class KeyValOffset[K, V](key: K, value : V, topicPartition : Int, 
		  						startOffset : Long, endOffset : Long) extends Serializable
  ...
  val kafkaStreamWithOffsets = kafkaInputStream.transform((rdd, _time) => {
    ...
    rdd.mapPartitionsWithIndex { (topicPartition, iter) =>
      ...
      // the offsets like this then that should be the first operation on the DStream. Nevertheless, it seems
      // we can obtain the partition from the OffsetRange too. But if we were applying this transform() on a partition() of
      // kafkaInputStream then we wouldn't be able to determine which i to use for getting the right OffsetRange from
      // offsets: in fact in that case probably the cast rdd.asInstanceOf[HasOffsetRanges] would fail, and this is the only way
      val osr: OffsetRange = offsets(topicPartition)

      // get any needed data from the offset range
      // val topic = osr.topic
      // val kafkaPartitionId = osr.partition
      val startOffset = osr.fromOffset
      val endOffset = osr.untilOffset
      iter.map { case (k, v) =>  KeyValOffset(k, v, topicPartition, startOffset, startOffset) }
    }
   }
  )
